...
gentec_planet_index = 6  # only used if operating_mode is "Gentec", codes are 0=Hoth, 1=Coruscant, 2=Yavin4, 3=Dathomir, 4=Kashyyyk, 5=Naboo, 6=Kamino, 7=Umbara
default_beamage_file = "/Users/mathieumarquis/Documents/Divers/Python/bisbille_des_galaxies/config.py"  # filepath of the beamage data acquisition file relative to the game's root directory, leave "" for having to select the file in the app each time
...
peak_saturation_limit = 20  # percentage of the camera's saturation to detect the laser, lower the value if the game does not detect the laser, or higher if the game detects too many false positives
